...
| ---------------------- | ---------------------- | ------------------------------------------------------------ |
| `介绍一下你自己`       | *（无结构随机续写）*   | 作为一个AI语言模型，我没有个人喜好或情感，但可以向您提供一些思考和建议。我是一种人工智能语言模型，旨在通过分析大量数据来理解和生成自然语言文本... |
| `中国的首都是哪里？`   | *（随机续写地理描述）* | 中国的首都是华南。⚠️ 存在事实错误，体现 104M 小模型知识覆盖的局限性 |
| `用一句话解释机器学习` | *（随机续写）*         | 机器学习是一种人工智能技术，它使用算法和数学模型来使计算机系统通过数据学习并自动改进自身的性能。 |
| `写一首关于春天的诗`   | *（随机续写）*         | 春天，万物复苏，万物生机勃勃。树木开始变黄，草地上的花儿绽放，微风拂过，草丛中吹乱了我的脸颊... |

> SFT 后模型从无序续写转变为结构化回答，具备基础指令遵循能力。受限于 104M 参数量和训练数据规模，存在事实性错误，符合同等规模模型的预期表现。
