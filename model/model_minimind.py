...
class MiniMindConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        # 外推长度 = factor * original_max_position_embeddings = 32K
        self.rope_scaling = {
            "beta_fast": 32,
            "beta_slow": 1,
            "factor": 16,
        ...
        self.num_experts_per_tok = num_experts_per_tok # 每个token选择专家的数量
        self.n_routed_experts = n_routed_experts # 总的专家数量
        self.n_shared_experts = n_shared_experts # 共享专家
        self.scoring_func = scoring_func # 评分函数
        self.aux_loss_alpha = aux_loss_alpha # 辅助损失的alpha参数
        self.seq_aux = seq_aux # 是否在序列上计算辅助损失
...
def precompute_freqs_cis(dim: int, end: int = int(32 * 1024), rope_base: float = 1e6,
    ...
    if rope_scaling is not None:
        ...
        if end / orig_max > 1.0:
            # YaRN: f'(i) = f(i)((1-y) + y/s)， y is linear ramp [0,1]
            # 计算频率维度边界
...
def repeat_kv(x: torch.Tensor, n_rep: int) -> torch.Tensor:
    ...
    if n_rep == 1:
        return x
    return (
        x[:, :, :, None, :].expand(bs, slen, num_key_value_head, n_rep, head_dim).reshape(bs, slen, num_key_value_head * n_rep, head_dim)
    )
...
class Attention(nn.Module):
    ...
    def forward(self,
                x: torch.Tensor,
                position_embeddings: Tuple[torch.Tensor, torch.Tensor], #  接受cos和sin
